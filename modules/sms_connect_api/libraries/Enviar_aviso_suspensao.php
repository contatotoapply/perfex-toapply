...
class Enviar_aviso_suspensao extends Abstract_lembrete
{
    public $ci;
    public $column_name_qtd_dias = 'cn_aviso_suspensao_qtd_tentativas';
    public $reminder_type        = 'aviso_servico_suspenso';
    ...
    public function __construct()
    {
        ...
        $this->qtd_dias_suspensao = get_option('sms_notifications_zap_engine_library_qtd_dias_para_enviar_lembrete_suspensao');
    ...
    public function enviar_lembrete()
    {
        ...
        if (date('H') >= 7) {
            ...
            $this->ci->db->select($select)
                ...
                ->join(db_prefix() . 'central_notificacoes_lembretes t3', 't1.id = t3.rel_id AND t3.reminder_type = "aviso_servico_suspenso" AND t3.date="'.$hoje.'"', 'left')
                ->where('t1.duedate IS NOT NULL', null, null)
                ->where('t3.date IS NULL')
                ->where('t1.cn_aviso_suspensao_qtd_tentativas <=', count($qtd_dias_suspensao));
            ...
            $description = "Enviou whatsapp sobre aviso de suspensão para ";
                ...
                if (!empty($contacts)) {
                    ...
                    foreach ($contacts as $contact) {
                        ...
                        if (!empty($phonenumber)) {
                            ...
                            $res = $gateway->trigger('notifications_zap_engine_servico_suspenso', $phonenumber, $merge_fields);

                            if ($res) {
                                $this->ci->invoices_model->log_invoice_activity($invoice->id, $description . " " . $phonenumber. " Contato: ". $contact['firstname']);
                                log_activity('Lembrete | Mensagem enviada para '. $phonenumber. " em ". date('Y-m-d H:i:s') . ' [Fatura ID ' . $invoice->id . ']. Lembrete de suspensão');
                            } else {
                                log_activity('Lembrete | Mensagem não enviada para '. $phonenumber. " em ". date('Y-m-d H:i:s') . ' [Fatura ID ' . $invoice->id . ']. Lembrete de suspensão'. json_encode(['res' => $res]));
                            }

                        } else {
                            log_activity("Lembrete | MENSAGEM NÃO ENVIADA, pois o cliente não tem um telefone válido. [Fatura ID {$invoice->id}]. Lembrete de suspensão| Email do Contato: " . $contact['email']);
                        }
                    }
                } else {
                    log_activity("Lembrete | MENSAGEM NÃO ENVIADA, pois o cliente não tem um telefone válido. [Fatura ID {$invoice->id}]. Lembrete de suspensão.");
                }
            }
        }
    }
