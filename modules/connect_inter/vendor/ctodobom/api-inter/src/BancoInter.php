...
class BancoInter
{
    ...
    public function controllerPostWithJson(
        string $url,
        array $data,
        array $http_params = null,
        bool $postJson = true
        ...
        if ($postJson) {
            ...
            $prepared_data = http_build_query($data);
        }
     ...
     * @return \stdClass
     */
    public function controllerPatch(
        string $url,
        $data,
        array $http_params = null,
        bool $postJson = true
     ...
     * @return [type]
     */
    public function getCobrancas(string $dataInicial = null, string $dataFinal = null)
    {
        try {
            ...
            $reply = $this->controllerGet("/cobranca/v3/cobrancas?dataInicial={$dataInicial}&dataFinal={$dataFinal}");
     ...
     * @return string Caminho completo do arquivo baixado
     */
    public function mostraPdfBrowser(string $nossoNumero)
    {
     ...
     * @return [type]
     */
    public function cancelarBoleto($url, $data)
    {
        $this->checkOAuthToken();
        ...
        // Prepara os dados para envio se existirem
        $prepared_data = $data ? json_encode($data) : null;

        // Inicializa o cURL
        $curl = curl_init();

        curl_setopt($curl, CURLOPT_URL, $this->getApiBaseURL() . $url); // Adiciona a URL para a requisição
        curl_setopt($curl, CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4);
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 2);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, true);
        curl_setopt($curl, CURLOPT_HEADER, true); // Para capturar o cabeçalho na resposta
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true); // Para capturar a resposta da requisição
        curl_setopt($curl, CURLOPT_SSLCERT, $this->certificateFile);
        curl_setopt($curl, CURLOPT_SSLKEY, $this->keyFile);
        curl_setopt($curl, CURLOPT_CAPATH, "/etc/ssl/certs/");
        ...
        curl_setopt($curl, CURLOPT_CUSTOMREQUEST, 'POST'); // Define o método como POST

        // Se houver dados para enviar, adicionar no corpo da requisição
        if ($prepared_data) {
        ...
        // Captura o código HTTP de resposta
        $http_code = curl_getinfo($curl, CURLINFO_HTTP_CODE);
        $header_size = curl_getinfo($curl, CURLINFO_HEADER_SIZE);
        $reply = substr($curlReply, $header_size);

        curl_close($curl);

        // Depuração: Exibe o código de resposta HTTP e o corpo da resposta
        ...
        return ['http_code' => $http_code, 'reply' => $reply];
    }
