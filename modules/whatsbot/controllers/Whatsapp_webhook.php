...
class Whatsapp_webhook extends ClientsController
{
    ...
    public function store_bot_messages($data, $interactionId, $rel_data, $type, $response)
    {
        ...
        if (!$option && !empty($data['filename']) && in_array('.' . get_file_extension($data['filename']), array_map('trim', explode(',', $extensions['image']['extension'])))) {
            $header_image = '<a href="' . base_url(get_upload_path_by_type($type) . $data['filename']) . '" data-lightbox="image-group"><img src="' . base_url(get_upload_path_by_type($type) . $data['filename']) . '" class="img-responsive img-rounded" style="width: 300px"></img></a>';
        }
        if (!$option && !empty($data['filename']) && in_array('.' . get_file_extension($data['filename']), array_map('trim', explode(',', $extensions['document']['extension'])))) {
            $header_image = '<a href="' . base_url(get_upload_path_by_type($type) . $data['filename']) . '" target="_blank" class="btn btn-default tw-w-full">' . _l('document') . '</a>';
        }
        if (!$option && !empty($data['filename']) && in_array('.' . get_file_extension($data['filename']), array_map('trim', explode(',', $extensions['video']['extension'])))) {
            $header_image = '<video src="' . base_url(get_upload_path_by_type($type) . $data['filename']) . '" controls class="rounded-lg max-w-xs max-h-28"></video>';
        }
        if (!$option && !empty($data['filename']) && in_array('.' . get_file_extension($data['filename']), array_map('trim', explode(',', $extensions['audio']['extension'])))) {
            $header_image = '<audio controls class="w-[250px]"><source src="' . base_url(get_upload_path_by_type($type) . $data['filename']) . '"  type="audio/mpeg"></audio>';
        }
        $buttonHtml .= '</div>';
